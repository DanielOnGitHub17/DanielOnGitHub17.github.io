...
let mhud = {
    general: {
        det: 'For the Matrix in general:<br>Determinant: '
        , kind: '<br>Type of Matrix: '
        , dim: '. Dimension: '
    },
    specific: {
        row: 'Selected:<br>Position of selected: Row '
        , col: ', Column '
        , minor: '<br>Minor of selected: '
        , cofactor: '<br>Co-factor of selected: '
        , sign: '<br>Sign of selected: '
    }
}
    , allops = ['+', '-', 'X', 'transpose', 'minor', 'cofactor', 'adjoint', 'inverse']
    , t2s = ['second', 'answer']
    , two = allops.slice(0, 3)
    , one = allops.slice(3)
    , h = ['operation', 'row', 'col']
    , rc = h.slice(1)
    , pm = allops.slice(0, 2)
    , square = allops.slice(4)
    , n = [4, 3, 2, 1, '?'];
    
class Matrix {
    constructor(dim) {
        this.dim = dim;
        this.out = this.dim.x != this.dim.y;
        this.space = make();
        this.space.className = 'space';
        MATRICES.appendChild(this.space);
    ...
    help() {
        ...
        h.forEach(i => {
            this.space[i] = make('select');
            this.space.appendChild(this.space[i]);
        });
        t2s.forEach(i => {
             this.space[i] = make('table');
             this.space.appendChild(this.space[i]);
             this.space[i].className = i 
        });
        this.space.solve = make('button');
        this.space.solve.disabled = 1;
        this.space.insertBefore(this.space.solve, this.space.answer);
        //adding content (and options)
        allops.forEach(i => {
                let x = make('option');
                x.textContent = i;
                x.disabled = this.out && square.includes(i);
        ...
        this.space.operation.onchange = () => {
            ...
            if (pm.includes(op)){
                this.fromArray(copy(this.content, 0), this.space[t2s[0]]);
            }
        }
        this.space.solve.onclick = (dd) => {
            ...
            , v = this.space.operation.value;
            this.fromArray(this[dec[v] ? dec[v] : v], this.space.answer, 0, 0);
            dd.target.disabled = true;
        }
        this.space.col.onchange = () => {
            let array = []
            , x = this.space.row.value
            , y = this.space.col.value;
    ...
    fromArray(array, table, clear = false, edit = true) {
        //clearing
        Array.from(table.children).forEach(i => i.remove());
        table.content = [];
        if (clear) return;
        //building (maybe do a subclass later)
        table.content = array;
        table.parentElement.row.value = array.length;
        table.parentElement.col.value = array[0].length;
        let x = 0
        , y = 0;
        array.forEach(r => {
            ...
            r.forEach(c => {
                ...
                let inp = make('input'); inp.type = 'number';
                inp.value = array[x][y];
                co.appendChild(inp);
                inp.disabled = !edit;
                let rw = x
    ...
    get kind() {
        ...
        if (x == 1) kind.push('row');
        if (y == 1) kind.push('column');
        if (this.out) return kind;
        kind.push((this.det == 0 ? '' : 'non-') + 'singular');
        if (x == y) kind.push('square');
        let unidi = [true, true];
        for (let i = 0; i < x; i++) {
            for (let j = 0; j < y; j++) {
        ...
        if (unidi[0]) kind.push('diagonal');
        if (unidi[1]) kind.push('unit');
